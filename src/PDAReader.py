...
def read_PDA(PDA_Path):
    ...
    with open(PDA_Path, 'r') as PDA_file:
        ...
        while (line != ""): # kosong artinya end of file
            ...
            if (phase == 1): # baca states
                states_set += line
            elif (phase == 2): # baca input alphabet
                input_alphabet += line
            elif (phase == 3): # baca stack alphabet
                stack_alphabet += line
            elif (phase == 4): # baca start state
                if (len(line) > 0):
                    start_state = line[0]
            elif (phase == 5): # baca start stack
                if (len(line) > 0):
                    start_stack = line[0].split('_')
            elif (phase == 6): # baca final states
                final_states += line
            else: # baca transition functions
                if (len(line) > 4):
